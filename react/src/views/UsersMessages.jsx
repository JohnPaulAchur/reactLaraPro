...
import UsersMessagesDataTable from "../components/UsersMessagesDataTable";
...
export default function UsersMessages(){
  ...
  //     getMessages(); // Fetch messages every 5 seconds
  //   }, 30000);

  const getMessages = () => {
    setLoading(true);
    axiosClient.get(`/users-messages?page=${currentPage}&per_page=${perPage}`)
      .then(({ data }) => {
        setLoading(false);
